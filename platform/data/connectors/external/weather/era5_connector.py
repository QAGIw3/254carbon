"""
ECMWF ERA5 Reanalysis Connector

Coverage: Hourly global reanalysis.
Docs: https://www.ecmwf.int/en/forecasts/datasets/reanalysis-datasets/era5
...
class ERA5Connector(Ingestor):
    """ERA5 reanalysis connector."""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        # Use CDS API (cdsapi) with ERA5 datasets: 'reanalysis-era5-single-levels' or 'reanalysis-era5-land'
        self.dataset = config.get("dataset", "reanalysis-era5-single-levels")
        # Variables: accept long names per CDS; we'll map to t2m/u10/v10/tp internally via NetCDF
        ...
        # Area bbox [N, W, S, E] or point (lat, lon)
        ...
        # Date range (YYYY-MM-DD) and hours list
        ...
        # Live toggle and optional cds credentials (fallback to ~/.cdsapirc)
    ...
    def discover(self) -> Dict[str, Any]:
        return {
            ...
            "streams": [
                ...
                {"name": "u10/v10", "market": "weather", "product": "wind_ms", "unit": "m/s", "update_freq": "hourly"},
    ...
    def pull_or_subscribe(self) -> Iterator[Dict[str, Any]]:
        if not self.live:
            ...
            yield {"timestamp": now, "entity": self._entity_label(), "variable": "t2m_c", "value": 14.9, "unit": "C"}
            yield {"timestamp": now, "entity": self._entity_label(), "variable": "wind_ms", "value": 6.2, "unit": "m/s"}
            yield {"timestamp": now, "entity": self._entity_label(), "variable": "precip_mm", "value": 0.6, "unit": "mm"}
            return

        # Live mode: CDS → NetCDF → parse
        try:
            ...
            logger.error(f"ERA5 live mode requires cdsapi and netCDF4 installed: {e}")
            now = datetime.utcnow().isoformat()
        ...
        with tempfile.TemporaryDirectory() as td:
            out_path = os.path.join(td, "era5.nc")
            ...
            try:
                ...
                def spatial_mean(varname: str, idx: int) -> float:
                    ...
                    # Reduce spatial dims via mean to avoid high fanout
                    try:
                        return float(arr.mean())
                    except Exception:
                        # scalar or masked scalar
                        return float(arr)
    ...
    def map_to_schema(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        instrument = f"ERA5.{entity}.{variable.upper()}"
        return {
    ...
    def emit(self, events: Iterator[Dict[str, Any]]) -> int:
        ...
        if self.producer is not None:
            self.producer.flush()
        logger.info(f"Emitted {count} ERA5 events to {self.kafka_topic}")
        return count

    def checkpoint(self, state: Dict[str, Any]) -> None:
        self.checkpoint_state = state
        logger.debug(f"ERA5 checkpoint saved: {state}")
...
if __name__ == "__main__":
    connector = ERA5Connector({"source_id": "era5_reanalysis"})
    connector.run()
