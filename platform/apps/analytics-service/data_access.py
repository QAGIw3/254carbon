"""Centralized data access helpers for Analytics service modules.

Copied and adapted from ml-service to ensure parity across services.
"""
...
class DataAccessLayer:
    ...
    def get_price_dataframe(
        ...
        *,
        ...
        pivot = df.pivot_table(index="event_time", columns="instrument_id", values="value").sort_index()
        return pivot
    ...
    def _fetch_series(
        ...
        *,
        ...
        if not rows:
            if required:
                logger.debug("No rows returned for %s where %s", table, filters)
            return pd.Series(dtype=float)
